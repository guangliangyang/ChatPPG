...
parser.add_argument('--llm_path', type=str, default='/root/autodl-tmp/llama-7b', help='llm model folder')
#输入的features
...
# model define
parser.add_argument('--enc_in', type=int, default=7, help='encoder input size') # number of input features
...
parser.add_argument('--d_ff', type=int, default=32, help='dimension of fcn') # hidden layer size in the feed-forward network
...
parser.add_argument('--llm_dim', type=int, default='4096', help='LLM model dimension')# LLama3.2-1b:2048;  LLama7b:4096; GPT2-small:768; BERT-base:768, qw 1536
...
parser.add_argument('--llm_layers', type=int, default=6) #lama3.2-1b 16
parser.add_argument('--percent', type=int, default=100)
parser.add_argument('--count_hyper', type=int, default=0)
parser.add_argument('--down_proj', type=int, default=1)
...
for ii in range(1):
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader) , disable=True ):
            ...
            # encoder - decoder
            if args.use_amp: #支持自动混合精度（AMP）加速训练
                with torch.cuda.amp.autocast():#采用 混合精度（FP16 + FP32）
                    if args.output_attention:
                        # batch_x : 时间序列数据，形状 (batch_size, time_steps, num_features)
                        # batch_x_mark: 时间戳编码（日期）
                        # dec_inp: 解码器输入（通常是目标时间序列的部分数据）
                        # batch_y_mark: 目标序列的时间
            ...
            elapsed_time = end_time - start_time  # 计算时长
            #print(f"ricky-Execution Time: {elapsed_time:.6f} seconds")
